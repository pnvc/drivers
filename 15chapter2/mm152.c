...
static int dev_reg(struct mm152 *mm152)
{
	...
	if (err) {
		pr_info(MM152M "alloc_chrdev_region failed" F_L);
		return err;
	}
...
static int cdev_reg(struct mm152 *mm152)
{
	...
	if (err) {
		pr_info(MM152M "cdev_add failed" F_L);
		cdev_del(&mm152->cdev);
		return err;
	}
...
static int mm152_class_create(struct mm152 *mm152)
{
	...
	if (!mm152->class) {
		pr_info(MM152M "class_create failed" F_L);
		return -ENOMEM;
	}
...
static int mm152_device_create(struct mm152 *mm152)
{
	if (!device_create(mm152->class, NULL, mm152->dev, NULL, MM152)) {
		pr_info(MM152M "device_create filed" F_L);
		return -ENOMEM;
	}
